...
如果 `pet_data.json` 里只是某个字段的类型不对（例如优先级被手动改成了文字），
启动时会自动把它归一化为安全值，不会因为一条坏记录让整个工作台打不开；设置页
会显示一次修复提示。完整 JSON 损坏仍会优先从 `pet_data.backup.json` 恢复。
...
# 2. 打包（约 8 分钟：53 个套件 + PyInstaller + 装出来的包真跑一遍）
.venv\Scripts\python.exe tools\build.py --installer --zip
...
- **专注** — 大圆环倒计时、开始/暂停/重置/跳过、15/25/45/60/90 分钟快捷选择、自动衔接下一阶段；
  每轮可以关联一个未完成待办，完成后自动给该待办记 1 个番茄，任务列表里的番茄数会同步更新。
- **待办** — 优先级（普通/重要/紧急，创建后也能调整）、到期日、双击改名、勾选完成、三种排序、
  关键词搜索、只看待办或看全部、清除已完成（会二次确认）。单条误删后 6 秒内可撤销，
  **多少条都能滚动看到。**
- **便签** — 左边列表、右边编辑，**输入即自动保存**；误删后 6 秒内可以撤销，
  `Ctrl + S` 也能立即保存，`Ctrl + N` 可快速新建。
- **提醒** — 定时提醒（仅一次 / 每天 / 工作日 / 每周 / 每 N 分钟）+ 久坐提醒；误删后 6 秒内可撤销。
- **AI 操作** — 让 AI 看屏幕、替你操作电脑；删除历史对话、清空记忆或知识库前会先确认范围。
...
待确认时工作台会自动把对应卡片滚到右栏可视区域，提问、危险审批和批量确认的按钮不会藏在长对话下面；窗口较窄时也会优先保证操作按钮可见。
...
tools/
  regress.py             全量回归入口：53 个套件，是唯一的测试清单
  ...
  corruptdatatest.py     损坏字段归一化 + Backend 启动容错
  ...
  approvalviewtest.py    待确认卡片的可见性与自动定位回归
