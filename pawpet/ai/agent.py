...
class AgentRunner:
    ...
    def run(self, user_text: str) -> str:
        ...
        #
        # **只放进 last_summary，不拼进 final_text。**
        #
        # 这是用户实际报过的问题：截图里同一段话出现了**两遍** ——
        # 一遍在助手气泡正文里（因为拼进了 final_text），一遍在气泡底部
        # 那行小字里（界面读的就是 report 字段）。两个都显示，自然重复。
        #
        # 而且拼进正文还有个更糟的后果：界面里那行小字用 reportText 的
        # 高度算气泡高度，正文再变长就会互相顶，实测出现文字**溢出气泡**、
        # 被宠物挡住。
        #
        # 所以分工要清楚：final_text = 模型要说的话，
        # last_summary = 执行层统计的附注，界面把两者分开放。
        #
        # 两个条件：
        # * 这一轮**真的动过手** —— 纯问答（「这段报错什么意思」）后面
        #   跟一句「执行了 0 个操作」很怪；
        # * 必须有**正文**。否则交代会变成唯一内容，那就是用户看到的
        #   「只显示『这一轮执行了 1 个操作』，任务根本没完成」。
    ...
    def _force_answer(self) -> str:
        ...
        『这一轮执行了 1 个操作』」，完全不知道发生了什么。这是用户报过的
        「任务没有完成就结束了」。
        ...
        注意第 2 种情况下，这句话会**同时**成为正文和 last_summary，
        界面上会显示两遍（正文 + 底部那行小字）。这是刻意的：
        * 正文不能空 —— 空了就是用户报的「什么都没有了」；
        * 底部那行小字也不能空 —— 它是「干了什么、文件在哪」的固定位置。
        内容确实重复，但空着比重复糟得多。第 1 种情况（补问成功）
        不会重复，那是绝大多数时候。
        """
        try:
            self._append_user(
